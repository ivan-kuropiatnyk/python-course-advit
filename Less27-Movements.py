...
myimage = pygame.transform.scale(myimage,(image_size,image_size)) #transforms loaded image to dimensions 100x100
...
while game_over == False:#during a game_over is't set
    ...
    for event in pygame.event.get():#receive some event
        ...
        if event.type == pygame.KEYUP:#event 2 unpressed button
             if event.key == pygame.K_LEFT:#if unpressed LEFT button stop to move image
                move_left = False
            if event.key == pygame.K_RIGHT:
                move_right = False
            if event.key == pygame.K_UP:
